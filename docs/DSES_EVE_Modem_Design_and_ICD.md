...
| Interface | Specification |
|---|---|
| Keying output | One logic line from the B210 GPIO header: J504, the 10-pin 0.1-inch (2.54 mm) header on the board (Ettus rev 6 and later), pins 1 to 8 = GPIO_0 to GPIO_7, pins 9 and 10 = ground, 3.3 V LVCMOS. The modem drives GPIO_0 = pin 1 (UHD bank FP0, line 0); return on pin 9 or 10. The Ettus enclosure brings no GPIO outside the case, so the station either routes J504 to a panel connector or keys from the host through a USB relay (the keying is host-timed, D22, so a USB relay loses nothing); the DSES clone's front-panel IO connector is that header brought out as a white 2x5 shrouded header (2.54 mm pitch; a standard 10-pin IDC ribbon socket or 2.54 mm Dupont leads fit) labeled with the GPIO numbers, top row G 6 4 2 0 and bottom row G 7 5 3 1, so the key line there is the pin marked 0 and a G. The line goes through an isolated driver (opto-coupler or relay), never straight to the sequencer. Implemented (2026-09-13, `eve/keyer.py`): the session keys through a Keyer object, either the GPIO line or an LCUS-type USB serial relay board (CH340, 9600 baud, four-byte frames; the station's choice, ordered 2026-09-13: a 2-channel USB-C board, `docs/hardware/usb_relay_keyer.md`), selected on the application's Setup tab; a keying fault is logged and shown, and the run continues under the operator's eye. Asserted for the whole on-window. The line is driven host-timed from the session's keyer thread (UHD's timed GPIO commands did not defer on the bench build); with T_lead of 200 ms, host timing is more than adequate, and a watchdog drops the line if a chunk overruns |
