...
| Revision | Rev B — DRAFT (program 1.0.4) |
| Date | 2026-09-14 |
...
| Bench margins near 40 dB with pilot contrast in the thousands; the report says "loopback level far above internal leakage" | The transmitter is feeding the receiver directly: that frontend's TX/RX and RX2 ports are shorted (failed T/R switch; seen on an old B210 marked RFA RX2 bad). Never put that frontend on the air. Switch both TX and RX to the other frontend under Ports, and confirm with a signal generator into each port. |
